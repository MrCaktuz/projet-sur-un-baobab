...
?>
    ...
    <aside class="sideNav">
        <h2 class="sideNav__titles">Nos tutoriels</h2>
        <ol class="sideNav__list">
            <?php
                $posts = new WP_QUERY( [ 'post_type' => 'tuto' ] );
                if ( $posts -> have_posts() ): while ( $posts -> have_posts() ): $posts -> the_post();
            ?>
                <li class="sideNav__listElts">
                    <a href="the_permalink();" class="sideNav__links links"><?php the_title(); ?></a>
                </li>
            <?php endwhile; endif; ?>
        </ol>
    </aside>
    <div class="bigWrap">
        <h2 class="archives__title">Nos tutoriels</h2>
        <?php
            $posts = new WP_QUERY( [ 'post_type' => 'tuto' ] );
            if ( $posts -> have_posts() ): while ( $posts -> have_posts() ): $posts -> the_post();
        ?>
